from team_7k7k.board import Board
from team_7k7k.rules import check_if_steal
from team_7k7k.agent import choose_action
...
class Player:
    ...
    def action(self):
        ...
        if(self.player=='blue' and self.round==1):
            # check if blue do not steal red pieces in the frist turn:
            if(self.n<5):
                action = check_if_steal(self.board,self.player)
            else:
                action = ('PLACE',self.n//2 , self.n//2)
